...
const SUN = 0, MON = 1, TUES = 2, WED = 3, THUR = 4, FRI = 5, SAT = 6;
...
let dayInMonthHebrewLatter = [
    "א","ב","ג","ד","ה","ו","ז","ח","ט","י"
    ,"יא","יב","יג","יד","טו","טז","יז","יח","יט","כ"
    ,"כא","כב","כג","כד","כה","כו","כז","כח","כט","ל"
];
let yechidotEbrewCharacters = [
    "","א","ב","ג","ד","ה","ו","ז","ח","ט"
];
let arryDozensAsEbrewCharacters = [
    "","י","כ","ל","מ","נ","ס","ע","פ","צ"
];
let gWeekdayHebrew = [
    "ראשון", "שני", "שלישי", "רביעי", "חמישי", "שישי", "שבת"
];
function HundredsAsEbrewCharacters(yearAsString){
    switch(yearAsString){
        case  "0" :
            return ""
        case  "1" :
...
const gWeekday = new weekdayarr("Sun", "Mon", "Tues", "Wednes", "Thurs", "Fri", "Satur")
    ...
    , hHebrewMonth = new hebrewmontharr("תשרי", "מר חשון", "כסלו", "טבת", "שבט", "אדר ראשון", "אדר שני", "ניסן", "אייר", "סיון", "תמוז", "מנחם אב", "אלול")
...
module.exports = function (inputDateOrYear, inputMonth, inputDate) {
    ...
    if (typeof inputYear === "object") {
        ...
        DayInWeekName = gWeekdayHebrew[inputDateOrYear.getDay() + 1];
    }
    else{
        DayInWeekName = gWeekdayHebrew[(new Date(inputYear, inputMonth, inputDate)).getDay()];
    }
    
    SdnToHebrew(GregorianToSdn(inputYear, inputMonth, inputDate));
    yyHelp = hebrewYear.toString();
    yyHebrewYear = yechidotEbrewCharacters[yyHelp[0]]+ HundredsAsEbrewCharacters(yyHelp[1]) +arryDozensAsEbrewCharacters[yyHelp[2]]+ yechidotEbrewCharacters[yyHelp[3]];
    
    if(!isMeuberetYear)
    {
        hHebrewMonth[hebrewMonth - 1] = hHebrewMonth[hebrewMonth - 1].replace("ראשון", "");
        hMonth[hebrewMonth - 1] = hMonth[hebrewMonth - 1].replace("I", "");
    }

    return {
        day_in_week_name: DayInWeekName
      , day_in_month_num: hebrewDate
      , day_in_month_hebrew_letter : dayInMonthHebrewLatter[hebrewDate - 1]
      , month_name: hMonth[hebrewMonth - 1]
      , month_name_hebrew_letter: hHebrewMonth[hebrewMonth - 1]
      , month: hebrewMonth
      , year_hebrew_letter : yyHebrewYear
      , year: hebrewYear 
      , is_meuberet_year: isMeuberetYear
    };
};
